function [V,m,h,n]=Task2I2V_fast(C,E,g,Alpha,Beta,VPASol,options)
    ...
    arguments
        ...
        VPASol %初值VPASol=vpasolve(eqt,[V0 m0 h0 n0]);
...
% PPT中给出的微分方程是
    ...
    fV=@(V,m,h,n,I) 1/C*(-g.L*(V-E.L)-g.Na*m^3*h*(V-E.Na)-g.K*n^4*(V-E.K)+I);
    ...
    for i=1:N-1
        kV1=fV(V(i),m(i),h(i),n(i),I_ext(i));
        km1=fm(V(i),m(i));
        kh1=fh(V(i),h(i));
        kn1=fn(V(i),n(i));

        % 中点电流值
        I_mid = (I_ext(i) + I_ext(i+1))/2;

        kV2=fV(V(i)+Del/2*kV1,m(i)+Del/2*km1,h(i)+Del/2*kh1,n(i)+Del/2*kn1,I_mid);
        ...
        kV3=fV(V(i)+Del/2*kV2,m(i)+Del/2*km2,h(i)+Del/2*kh2,n(i)+Del/2*kn2,I_mid);
        ...
        kV4=fV(V(i)+Del*kV3,m(i)+Del*km3,h(i)+Del*kh3,n(i)+Del*kn3,I_ext(i+1));
