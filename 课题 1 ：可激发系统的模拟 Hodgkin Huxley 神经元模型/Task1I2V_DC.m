function [V,m,h,n]=Task1I2V_DC(C,E,g,Alpha,Beta,options)
% Task1I2V_DC相比于Task1I2V_DC修改了绘图，将电流刺激和膜电压画在了一起
    ...
    if IfPic==1
    % fprintf("\n下图是外部电流刺激的时域图\n")
    fprintf("直流_当A="+A+"时的外部电流刺激I-t和膜电压V-t图 \n")
    figure('Name',"当A="+A+"时的外部电流刺激I-t和膜电压V-t图")
    yyaxis left
    plot(t,I_ext(t),'r-')
    axis padded
    hold on
...
% PPT中给出的微分方程是
    ...
    if IfPic==1
        ...
        xlabel('t/ms')
        ylabel('V/mv')
        legend('外部电流刺激I-t','膜电压V-t','Location','best')
        % saveas
    end
end